...
{
    public sealed partial class WeekCalendar : UserControl, ICalendar
    {
        public Grid layout;
        ...
        public WeekCalendar(DateTime date, EventControler controler, ScrollViewer myScroll)
        {
            Date = date;
            Controler = controler;
        ...
        public WeekCalendar(DateTime date, ScrollViewer myScroll)
        {
            Date = date;
            this.InitializeComponent();
            Canvas = new Canvas[24, 7];
        ...
        private void canvas_PointerPressed(object sender, Windows.UI.Xaml.Input.DoubleTappedRoutedEventArgs e)
        {
            ...
            begin = Date.AddDays(Grid.GetColumn(beginCell) - 1).AddMinutes(-(Date.Minute)).AddSeconds(-(Date.Second)).AddHours(-(Date.Hour) + row);
            begin.AddHours(Grid.GetRow(beginCell));

            //end = Date.AddDays(Grid.GetColumn(beginCell) - 1).AddHours(Grid.GetRow(beginCell)+1);
            end = Date.AddDays(Grid.GetColumn(beginCell) - 1).AddMinutes(-(Date.Minute)).AddSeconds(-(Date.Second)).AddHours(-(Date.Hour) + row + 1);
            ...
            WeekEvent we = new WeekEvent(events);
